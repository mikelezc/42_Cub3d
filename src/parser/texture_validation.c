...
/*   Updated: 2024/08/04 18:43:25 by mlezcano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cb_check_valid_line(char *line, t_parse *data)
{
	...
	if (ft_strncmp(line, "NO ", 3) == 0
		...
		if (!cb_check_texture_dir(line, data))
			return (false);
	if ((ft_strncmp(line, "F ", 2) == 0) || (ft_strncmp(line, "C ", 2) == 0))
	{
		if (!cb_check_and_save_color(line, data))
			return (false);
	}
	else
		return (false);
...
{
	if (!cb_check_no_so_texture(line, data)
		|| !cb_check_we_ea_texture(line, data))
		return (false);
	return (true);
}
