...
/*   Updated: 2024/07/24 20:32:30 by ampjimen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	bonus_check_valid_line(char *line, t_parse *data)
{
	...
	if (ft_strncmp(line, "NO ", 3) == 0
		...
		if (bonus_check_texture_dir(line, data) == false)
			return (false);
	if ((ft_strncmp(line, "F ", 2) == 0) || (ft_strncmp(line, "C ", 2) == 0))
	{
		if (bonus_check_and_save_color(line, data) == false)
			return (false);
	}
	else
		return (false);
...
{
	if (bonus_check_no_so_texture(line, data) == false
		|| bonus_check_we_ea_texture(line, data) == false)
		return (false);
	return (true);
}
...
int	bonus_check_no_so_texture(char *line, t_parse *data)
{
	...
	if (ft_strncmp(line, "NO", 2) == 0)
	{
		...
		if (bonus_check_open_file(file) == true)
